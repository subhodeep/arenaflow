...
from app.api.deps import get_firestore_repo, get_gemini_service, get_grounding_service, get_pubsub_repo
from app.core.security import enforce_request_size, rate_limit, require_scheduler_or_staff, require_staff
...
router = APIRouter(prefix="/api/v1/ops", tags=["operations"], dependencies=[Depends(enforce_request_size), Depends(rate_limit)])


@router.post("/decision-support", response_model=OpsDecisionSupportResponse, dependencies=[Depends(require_staff)])
...
@router.post("/snapshot", response_model=OpsSnapshotResponse, dependencies=[Depends(require_scheduler_or_staff)])
async def snapshot(
