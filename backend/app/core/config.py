...
class Settings(BaseSettings):
    ...
    environment: Literal["local", "dev", "stage", "prod"] = Field(default="local", validation_alias="ENVIRONMENT")
    ...
    pubsub_topic_operations: str = Field(default="arenaflow-local-operations", validation_alias="PUBSUB_TOPIC_OPERATIONS")
    pubsub_topic_crowd: str = Field(default="arenaflow-local-crowd", validation_alias="PUBSUB_TOPIC_CROWD")
    pubsub_topic_sustainability: str = Field(default="arenaflow-local-sustainability", validation_alias="PUBSUB_TOPIC_SUSTAINABILITY")

    rate_limit_requests_per_minute: int = Field(default=60, validation_alias="RATE_LIMIT_REQUESTS_PER_MINUTE")
    ...
    allow_local_staff_bypass: bool = Field(default=False, validation_alias="ALLOW_LOCAL_STAFF_BYPASS")
    scheduler_service_account_email: str | None = Field(default=None, validation_alias="SCHEDULER_SERVICE_ACCOUNT_EMAIL")

    maps_api_server_key_secret_name: str | None = Field(default=None, validation_alias="MAPS_API_SERVER_KEY_SECRET_NAME")
    ...
    collection_venue_graphs: str = Field(default="venue_graphs", validation_alias="COLLECTION_VENUE_GRAPHS")
    collection_events: str = Field(default="events", validation_alias="COLLECTION_EVENTS")
    collection_crowd_zones: str = Field(default="crowd_zones", validation_alias="COLLECTION_CROWD_ZONES")
    collection_incidents: str = Field(default="incidents", validation_alias="COLLECTION_INCIDENTS")
    collection_assistant_sessions: str = Field(default="assistant_sessions", validation_alias="COLLECTION_ASSISTANT_SESSIONS")
    collection_ops_snapshots: str = Field(default="ops_snapshots", validation_alias="COLLECTION_OPS_SNAPSHOTS")
    ...
    def validate_deployed_settings(self) -> "Settings":
        ...
        if self.environment not in {"local", "dev"}:
            auth_values = [self.staff_auth_issuer, self.staff_auth_audience, self.staff_auth_jwks_url]
            if not all(auth_values):
                raise ValueError("Staff auth issuer, audience, and JWKS URL are required in stage/prod")
        return self
