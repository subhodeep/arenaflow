...
class SustainabilityService:
    def __init__(self, grounding_service: GroundingService, gemini_service: GeminiService, pubsub_repo: PubSubRepository):
    ...
    async def advise(self, request: SustainabilityAdviceRequest) -> SustainabilityAdviceResponse:
        ...
        try:
            await self.pubsub_repo.publish_sustainability_event({"type": "sustainability_advice", "venue_id": request.venue_id, "event_id": request.event_id, "intent": request.intent})
        except Exception:
            pass
        return response
