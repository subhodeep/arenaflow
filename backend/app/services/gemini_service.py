...
_PRIORITY_ALIASES = {"urgent": "critical", "severe": "critical", "normal": "medium"}
_TEXT_KEYS = (
    "title",
    "summary",
    "instruction",
...
class GeminiService:
    ...
    def _normalize_payload(
        ...
        primary_text = self._primary_text(payload)
        ...
        self._normalize_text_lists(payload, normalized)
        self._normalize_structured_lists(payload, normalized)
        return normalized

    def _primary_text(self, payload: dict[str, Any]) -> str | None:
        for key in ("answer", "recommendation", "response", "message", "summary", "content"):
            value = payload.get(key)
            if value is not None:
                return self._item_to_text(value)
        return None
    ...
    def _normalize_text_lists(
        ...
        for field_name in (
            ...
            normalized[field_name] = self._string_list(payload.get(field_name)) or normalized.get(
                field_name, []
            )
        ...
        if "estimated_total_minutes" in normalized:
            normalized["estimated_total_minutes"] = self._optional_int(
                normalized.get("estimated_total_minutes")
            )

    def _normalize_structured_lists(
        self,
        payload: dict[str, Any],
        normalized: dict[str, Any],
    ) -> None:
        if "route_steps" in normalized:
            normalized["route_steps"] = self._normalize_route_steps(
                payload.get("route_steps") or payload.get("steps") or normalized.get("route_steps")
            )
        if "options" in normalized:
            normalized["options"] = self._normalize_transport_options(
                payload.get("options")
                or payload.get("transport_options")
                or normalized.get("options")
            )
        if "action_cards" in normalized:
            normalized["action_cards"] = self._normalize_action_cards(
                payload.get("action_cards")
                or payload.get("actions")
                or normalized.get("action_cards")
            )
    ...
    def _normalize_confidence(self, value: Any, default: Any = "medium") -> str:
        ...
        if isinstance(value, str):
            ...
            if lowered in _CONFIDENCE_VALUES:
                return lowered
            try:
                value = float(lowered)
            except ValueError:
                return default if default in _CONFIDENCE_VALUES else "medium"
    ...
    def _normalize_priority(self, value: Any, default: Any = "medium") -> str:
        ...
        if isinstance(value, str):
            lowered = value.strip().lower()
            if lowered in _PRIORITY_ALIASES:
                return _PRIORITY_ALIASES[lowered]
            if lowered in _PRIORITY_VALUES:
                return lowered
        if isinstance(value, int | float):
            if value >= 0.9:
                return "critical"
            ...
            if value >= 0.4:
                return "medium"
            return "low"
        return default if default in _PRIORITY_VALUES else "medium"
        ...
        for item in items:
            if isinstance(item, str):
                ...
                instruction = (
                    ...
                    or item.get("action")
                    or "Proceed to the next waypoint."
                )
                steps.append(
                    {
                        "instruction": str(instruction),
                        "distance_meters": self._optional_int(
                            item.get("distance_meters") or item.get("distance")
                        ),
                        "duration_minutes": self._optional_int(
                            item.get("duration_minutes")
                            or item.get("minutes")
                            or item.get("duration")
                        ),
                        "accessibility_notes": self._string_list(
                            item.get("accessibility_notes") or item.get("accessibility")
                        ),
                        "crowd_notes": self._string_list(
                            item.get("crowd_notes") or item.get("crowd")
                        ),
                    }
                )
        return steps
    ...
    def _normalize_transport_options(self, value: Any) -> list[dict[str, Any]]:
        ...
        for item in items:
            if isinstance(item, str):
                ...
                carbon = str(item.get("carbon_impact") or item.get("carbon") or "unknown")
                options.append(
                    {
                        ...
                        "summary": str(
                            ...
                            or item.get("mode")
                            or "Recommended transport option."
                        ),
                        "estimated_minutes": self._optional_int(
                            item.get("estimated_minutes")
                            or item.get("minutes")
                            or item.get("duration")
                        ),
                        "carbon_impact": self._normalize_carbon_impact(carbon),
                        "accessibility_notes": self._string_list(
                            item.get("accessibility_notes") or item.get("accessibility")
                        ),
                    }
    ...
    def _normalize_carbon_impact(self, value: str) -> str:
        carbon = value.lower()
        return carbon if carbon in {"low", "medium", "high", "unknown"} else "unknown"
...
""".strip()
    ...
    async def chat_assistant(
        ...
        fallback = AssistantChatResponse(
            ...
            suggested_actions=[
                "Check nearby signage",
                "Ask the nearest volunteer for urgent support",
            ],
        )
        return await self._generate_json(
            self._base_prompt(
                "Answer a fan or staff question in the requested language.", request, context
            ),
            AssistantChatResponse,
            fallback,
        )
    ...
    async def generate_crowd_recommendation(
        ...
        fallback = CrowdRecommendationResponse(
            ...
            safety_notes=[
                "Escalate immediately if crowd movement becomes unsafe "
        ...
        return await self._generate_json(
            self._base_prompt(
                "Generate crowd management recommendations for staff.", request, context
            ),
            CrowdRecommendationResponse,
            fallback,
        )
    ...
    async def generate_accessibility_plan(
        ...
        return await self._generate_json(
            self._base_prompt("Create an accessibility support plan.", request, context),
            AccessibilityPlanResponse,
            fallback,
        )
    ...
    async def generate_transportation_plan(
        ...
        fallback = TransportationOptionsResponse(
            ...
            confidence="low",
            assumptions=[
                "Live AI transport plan unavailable; using general transportation fallback."
            ],
            safety_notes=[
            ...
            options=[
                TransportOption(
                    mode="public_transit",
                    summary=(
                        "Use official transit routes and allow extra time "
                        "around match start/end."
                    ),
                    carbon_impact="low",
                )
            ],
        ...
        return await self._generate_json(
            self._base_prompt("Recommend transportation options.", request, context),
            TransportationOptionsResponse,
            fallback,
        )
    ...
    async def generate_sustainability_advice(
        ...
        return await self._generate_json(
            self._base_prompt("Give sustainability advice.", request, context),
            SustainabilityAdviceResponse,
            fallback,
        )
    ...
    async def generate_ops_decision_support(
        ...
        fallback = OpsDecisionSupportResponse(
            ...
            escalation_guidance=(
                "Escalate to venue command for safety, security, medical, "
                "or blocked-egress concerns."
            ),
            alternatives=[
        ...
        return await self._generate_json(
            self._base_prompt("Generate real-time operational decision support.", request, context),
            OpsDecisionSupportResponse,
            fallback,
        )
